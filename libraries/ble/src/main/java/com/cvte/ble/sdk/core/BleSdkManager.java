...
public class BleSdkManager implements BleOperationListener {
    ...
    public static final String FILE_NAME = "CVTE_BLE_SDK_AIKA";
    ...
    public static BleSdkManager newInstance(Context context) throws NotSupportBleException {
        if (BleUtils.isBleSupported(context)) {
            ...
            throw new NotSupportBleException("Not Support Ble");
        }
    }

    @Override
    public boolean hasTargetDevice(String singleTag) {
        if (mAllDeviceMap == null) {
            return false;
        } else {
            return mAllDeviceMap.containsKey(singleTag);
        }
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (blueState) {
                case BluetoothAdapter.STATE_ON: {
                    ...
                    sHandler.removeCallbacks(mStopScanRunnable);
    ...
    public void init() {
        ...
        mSharedPreference = mContext.getSharedPreferences(FILE_NAME, Context.MODE_MULTI_PROCESS);
    }
    ...
    public void connectBleDevice(BleConnectInfo bleConnectInfo) {
        ...
        if (bleConnectDevice != null) {
            ...
            bleConnectDevice = new BleConnectDevice(mContext,null, bleConnectInfo);
    ...
    public void disConnectBleDevice(BleConnectInfo bleConnectInfo) {
        ...
        EventBleDevice eventBleDevice = new EventBleDevice(EventBleDevice.DISCONNECT_HANDLY, bleConnectInfo);
        EventBus.getDefault().post(eventBleDevice);
    ...
    private Runnable mScanDeviceRunnable = new Runnable() {
        ...
        public void run() {
            if (isConnectFinish) {
                stopScan();
                startScan();
                sHandler.postDelayed(mStopScanRunnable, BleSdkConfig.SCAN_TIME);
            }
        }
    };
    ...
    private Runnable mStopScanRunnable = new Runnable() {
        ...
        public void run() {
            stopScan();
            sHandler.postDelayed(mScanDeviceRunnable, BleSdkConfig.STOP_SCAN_TIME);
        }
    };
     ...
     * 当设备重启的时候缓存信息
     */
    public void saveAllDeviceInfoWhenShutDown() {
        JSONArray jsonArray = new JSONArray();
        Set<String> keySet = mAllDeviceMap.keySet();
        for (String tag : keySet) {
            BleConnectDevice connectDevice = mAllDeviceMap.get(tag);
            JSONObject jsonObject = new JSONObject();
            try {
                jsonObject.put("babyId", connectDevice.getBleConnectInfo().getSingleTag());
                jsonObject.put("imei", connectDevice.getBleConnectInfo().getVerifyCommand());
                jsonObject.put("imsi", connectDevice.getBleConnectInfo().getBroadCommand());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            jsonArray.put(jsonObject);
        }
//        BleLogUtils.LOGE(TAG, jsonArray.toString());
        if (disConnectAll()){
            setCacheDeviceInfo(jsonArray.toString());
        }
    }
...
//        BleLogUtils.LOGE(TAG, "cache data:" + info);
    }
    ...
    public String reloadCacheDeviceInfo() {
        ...
        String result =  mSharedPreference.getString(FILE_KEY, "");
//        BleLogUtils.LOGE(TAG,"reload data:"+result);
        return result;
    }
